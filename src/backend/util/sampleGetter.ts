...
>(
  ...
  return async (
    ...
    if (rawSample != null && !doesUpdate) {
      const adjustedSample = maker(rawSample, ...args.maker);
      await adjustedUtil.write(path, JSON.stringify(adjustedSample));
      return adjustedSample;
    } else {
      const rawSample = await getter(...args.getter);
      const promiseRaw = rawUtil.write(path, JSON.stringify(rawSample));
      const adjustedSample = maker(rawSample, ...args.maker);
